...
export const driveTypeEnum = pgEnum('drive_type', ['WITH_DRIVER', 'SELF_DRIVE'])
...
export const dateBlockSourceEnum = pgEnum('date_block_source', ['MANUAL', 'BOOKING'])
...
export const vendorProfiles = pgTable(
  ...
  (t) => ({
    ...
    verificationIdx: index('vendor_profiles_verification_idx').on(t.verificationStatus),
  })
)
...
export const vehicles = pgTable(
  'vehicles',
  {
    ...
    priceWithDriverDay: decimal('price_with_driver_day', { precision: 10, scale: 2 }),
    priceWithDriverMonth: decimal('price_with_driver_month', { precision: 10, scale: 2 }),
    priceSelfDriveDay: decimal('price_self_drive_day', { precision: 10, scale: 2 }),
    priceSelfDriveMonth: decimal('price_self_drive_month', { precision: 10, scale: 2 }),
  ...
  (t) => ({
    vendorSlugUnique: uniqueIndex('vehicles_vendor_slug_unique').on(t.vendorId, t.slug),
...
export const bookings = pgTable(
  'bookings',
  {
    ...
    parentBookingId: uuid('parent_booking_id').references(() => bookings.id),
...
export const chatThreads = pgTable(
  'chat_threads',
  {
    ...
    flaggedForContactLeak: boolean('flagged_for_contact_leak').notNull().default(false),
...
export const messages = pgTable(
  'messages',
  {
    ...
    blockedByContactRule: boolean('blocked_by_contact_rule').notNull().default(false),
...
export const incidents = pgTable(
  ...
  (t) => ({
    ...
    reviewedCreatedIdx: index('incidents_reviewed_created_idx').on(t.isReviewed, t.createdAt),
  })
)
...
export const vehicleDateBlocksRelations = relations(vehicleDateBlocks, ({ one }) => ({
  vehicle: one(vehicles, {
